export default {
  en: {
    title: 'Add device',
    titleChanged: 'Adding device ...',
    prompt:
      '1. Please keep the device in the connected state during the process of adding; \n2, During the process of adding, the device will no longer be used, please be patient. ',
    promptChanged: 'Here is the modified prompt',
    changeTitleAndStyle: 'Change title and style',
    changePromptAndStyle: 'Change prompt and style',
    changeProgressView: 'Change progress view color',
    changeIsCustomProgressChange: 'Change to custom progress change',
    backInitialState: 'Back to the initial state',
  },
  zh: {
    title: '添加设备',
    titleChanged: '添加设备中 ...',
    prompt: '1、添加过程中，请保持设备处于连接状态;\n2、添加过程中，设备将不能再使用，请耐心等待。',
    promptChanged: '这里是修改后的描述',
    changeTitleAndStyle: '修改标题文字和样式',
    changePromptAndStyle: '修改描述文字和样式',
    changeProgressView: '修改进度圆环颜色',
    changeIsCustomProgressChange: '改为自定义进度变化',
    backInitialState: '回到初始状态',
  },
};
