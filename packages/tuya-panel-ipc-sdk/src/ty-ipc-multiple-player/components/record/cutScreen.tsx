...
class CutScreen extends React.PureComponent<CutScreenProps, CutScreenState> {
  ...
  componentWillReceiveProps(nextProps: CutScreenProps) {
    const { baseImg, isVideoCut } = nextProps;
    if (baseImg && baseImg !== this.props.baseImg) {
      this.cutScreenListen({
        isVideoCut,
        imageSrc: baseImg,
      });
    }
  ...
  cutScreenListen = (data: { isVideoCut: boolean; imageSrc: string }) => {
    this.setState({
      showCutScreen: true,
