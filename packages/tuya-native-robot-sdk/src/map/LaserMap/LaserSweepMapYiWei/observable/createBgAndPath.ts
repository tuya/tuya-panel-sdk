...
import _, { isEqual, concat } from 'lodash';
...
interface StoreConfig {
  protocol?: any;
  mapProtocol: 'tuyaOss' | 'tuyaStream';
}

// 划分为三条channel: bucket, mapFilePath, pathFilePath,
export function createBgAndPathObservable(options: ParseMapFileUtilConfig, storeConfig?: StoreConfig) {
  ...
  const mapFilePathChannel$ = channel$
    .pipe(
      filter(createIsEqualType(fileTypeMap.map)),
      throttleTime(2000),
      map(getMapPath)
    )
    .do(d => {
  ...
  const pathFilePathChannel$ = channel$
    .pipe(
      filter(createIsEqualType(fileTypeMap.path)),
      throttleTime(2000),
      map(getMapPath)
    )
    .do(d => {
  ...
  const planPathFilePathChannel$ = channel$
    .pipe(
      filter(createIsEqualType(fileTypeMap.planPath)),
      throttleTime(2000),
      map(getMapPath)
    )
    .do(d => {
  ...
  const firstTuyaStream$ = fromPromise(Api.getTotalGyroStreamDataV3('1')).catch(extractError);

  const tuyaStreamNotification$ = (() => {
    const tuyaStreamState$ = Observable.fromEventPattern<{ state: boolean }>(
      ...
      .first();
    return tuyaStreamState$.switchMap(() => tuyaStreamMessage$);
  })();

  const tuyaStreamNotificationData$ = tuyaStreamNotification$.pipe(
    concatMap(() => fromPromise(Api.getTotalGyroStreamDataV3('1')).catch(extractError)),
    filter(isNotError)
  );

  const tuyaStreamData$ = firstTuyaStream$.concat(tuyaStreamNotificationData$);

  const tuyaStreamMapFileData$: Observable<IMapFileData> = tuyaStreamData$
    .pipe(
      map(({ data }) => {
  ...
  const pathIncrement$ = createDpValue$(pathDataCode, false)
    .pipe(
      filter(value => !!value),
      map(value => ASCIIToJson(value)),
  ...
  const fullAndIncrements$ = fullPathFileData$
    .switchMap((full: any) => Observable.of(full).concat(pathIncrement$))
    .do((d: any) => {
      logger.success('全量+增量所有路径的未累加数据流', d);
    }, handleError);

  const fullJoinIncrementsData$: Observable<IPathFileData> = fullAndIncrements$
    .pipe(
      scan((pre, cur, idx) => {
        if (idx === 0) {
          return cur;
  ...
  const requestIncrement$ = isSupportIncrementsPath()
    ? Observable.timer(0, 2000).pipe(
        withLatestFrom<number, string>(
          Observable.of(0).concat(fullJoinIncrementsData$.pipe(pluck('totalCount')))
        ),
  ...
  return {
    mapFileData$:
      (storeConfig && storeConfig.mapProtocol) === 'tuyaStream'
        ? tuyaStreamMapFileData$
        : tuyaOssMapFileData$,
    pathFileData$: isSupportIncrementsPath() ? fullJoinIncrementsData$ : fullPathFileData$,
    planPathFileData$,
    createHistoryFile$,
    mapFileUrl$,
    fullPathFileUrl$,
