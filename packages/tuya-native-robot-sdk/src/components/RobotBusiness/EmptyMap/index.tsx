...
import { TYText, Utils, IconFont } from '@tuya-rn/tuya-native-components';
...
export class EmptyMap extends PureComponent<IEmptyMapProps> {
  ...
  render() {
    const { icon, iconColor, iconSize, fontStyle } = this.props;
    return (
      <View style={[styles.root, this.props.style]}>
        <IconFont style={styles.icon} size={iconSize} color={iconColor} d={icon}></IconFont>
        <TYText style={[styles.textStyle, fontStyle]}>{i18n.getLang('emptyMap_title')}</TYText>
        <TYText style={[styles.textStyle, fontStyle]}>{i18n.getLang('emptyMap_subTitle')}</TYText>
      </View>
    );
  }
}
...
export class DrawMap extends PureComponent<IEmptyMapProps> {
  ...
  render() {
    ...
    return (
      <View style={[styles.root, this.props.style]}>
        <IconFont style={styles.icon} size={iconSize} color={iconColor} d={icon}></IconFont>
        <TYText style={[styles.textStyle, fontStyle]}>{i18n.getLang('drawMap_title')}</TYText>
      </View>
    );
  }
}
