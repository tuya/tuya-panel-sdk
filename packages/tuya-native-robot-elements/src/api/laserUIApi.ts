...
import { TYLaserManager, TYRCTTypeMapManager } from '../components/Basic/RCTLaserMap';
...
export interface ILaserMapAddAreaOpts extends ILaserMapOpts {
  area: ILaserMapArea | ILaserMapArea[];
}
...
function setLaserMapStateAndEdit(opts: ILaserMapStateAndEditOpts) {
  ...
  return new Promise((resolve, reject) => {
    const success = () => {
      // TYSdk.event.emit('TYSdkRobotElementEvent-LeDongLaserMapStore:UPDATE', {
      //   RCTMapState: state,
      // });
      resolve();
    };
...
function getLaserMapPointsInfo(
  opts: ILaserMapOpts
): Promise<{
  type: number;
  data:
  ...
  return new Promise((resolve, reject) => {
    if (!!mapId && isHasTYRCTTypeMapManager) {
      TYRCTTypeMapManager.getMapPointsInfo(mapId, (value: { data?: ILaserMapArea[], type: number }) => {
...
function setLaserMapSplitType(opts: ISetLaserMapSplitOpts) {
  return new Promise((resolve, reject) => {
    const { mapId = global.mapId, state } = opts;
...
function setLaserMapState(status: number, callback: Function) {
