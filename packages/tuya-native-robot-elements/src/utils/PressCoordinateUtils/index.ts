...
export function scaleNumber(scale: number, value: number) {
  return Number((value / 10 ** scale).toFixed(scale));
}

// 兼容负数 byte 最大值平分给正负两端
export function dealPL(point: number) {
  return point > min ? point - max : point;
}

// 放大x，y轴几倍
export function transformXY(mapScale: number, x: number, y: number) {
  return scaleNumber(mapScale, dealPL(highLowToInt(x, y)));
}

/** 通过比例尺，计算app x，y轴 */
export function transformXYByResolution(x: number, y: number, resolution: number) {
  // 乐动在用
...
export function transformPileXY(pileX: number, pileY: number, originX: number, originY: number) {
