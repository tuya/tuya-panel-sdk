 ...
 * @Date: 2021-11-15 20:57:07
 * @LastEditTime: 2021-11-16 17:03:09
 * @LastEditors: 豆芽(douya.ye@tuya.com)
 * @Description: 阴影组件
 * @FilePath: /tuya-panel-sdk/packages/tuya-panel-szos-sdk/src/components/box-shadow/index.tsx
 */
...
function colorRgb(color: string) {
  ...
  if (sColor && reg.test(sColor)) {
    ...
    for (let i = 1; i < 7; i += 2) {
      const d = `0x${sColor.slice(i, i + 2)}`;
      rgb.push(parseInt(d, 10));
    }
    return rgb;
  }
  return '#ffffff';
...
const BoxShadow: FC<BoxShadowProps> = (props: BoxShadowProps & { children: any }) => {
  const {
    setting: {
      width = 0,
      height = 0,
