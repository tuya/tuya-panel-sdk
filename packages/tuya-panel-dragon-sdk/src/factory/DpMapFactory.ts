...
export default {
      ...
      if (loop) {
        ...
        while (listLength >= 0 ? listLength > result[name].length : true) {
          ...
          if (value) {
            ...
            Object.assign(result, this._handleDefault(item));
          }
          if (done) {
            break;
          }
  ...
  _handleDefault(dpMapItem: DpMapItem) {
    const { name, type, loop, default: defaultValue } = dpMapItem;
    if (loop) {
      if (typeof defaultValue !== 'undefined') {
        return { [name]: defaultValue };
      } else {
        return { [name]: this._returnItemDefault(dpMapItem) };
      }
    } else {
      const defaultValue = this._returnItemDefault(dpMapItem);
      if (type === MapType.complex) {
        return defaultValue;
      } else {
        return { [name]: defaultValue };
      }
