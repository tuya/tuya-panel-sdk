...
export const sendDp = async (data: DpData, option: SendOption) => {
    ...
    if (now - lastSendTime <= throttleWaitTime) {
      ...
      return new Promise((resolve, reject) => {
        ...
        if (res.code === 'SENDED') {
          ...
          doBeforeCallback(lastWorker.data, lastWorker.willSendData);
          DpQueueManager.add(lastWorker.data);
  ...
  doBeforeCallback(worker.data, worker.willSendData);
  // 记录下发的 dp 数据
  DpQueueManager.add(worker.data);
 ...
 * @param data dp 协议数据
 */
export const parseDp = (data: DpData) => {
  const { dpMap = {} } = getConfig();
  const result: any = {};
  Object.keys(data).forEach(code => {
    let value = data[code];
 ...
 * @param data dp 对象数据
 */
export const formatDp = (data: DpData) => {
  const { dpMap = {} } = getConfig();
  const result: any = {};
  Object.keys(data).forEach(code => {
    let value = data[code];
    ...
    if (hasProp(dpMap, code)) {
      // 使用映射规则将标准dp协议数据转化为可描述dp对象数据
      value = DpMapFactory.format(dpMap[code], value);
    } else {
      // 初始化的数据为dp标准协议数据，需要转化为普通数据
      const formater = getFormater(code);
      if (formater) {
 ...
 * @param data dp 数据
 */
export const doBeforeCallback = async (data: any, willSendData: any) => {
  let result = willSendData;
  // 后注册的先跑，如果处理方法有返回数据，如果返回false， 则终断其他回调不执行，否则下一个监听器会使用此数据做为第一参数
  for (let i = beforeCallbacks.length - 1; i >= 0; i--) {
    const cb = beforeCallbacks[i];
    if (typeof cb === 'function') {
      const res = cb(result, gatwayDpData);
      if (res === false) {
        break;
      }
