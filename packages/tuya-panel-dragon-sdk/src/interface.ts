...
export interface Rule {
  ...
  /**
   * 规则条件
   * 当 type 为 FORBIDDEN 或 PASS 时，使用 DpData[] 类型，并会在当前所有 dp 数据中判断
  ...
  /**
   * 规则影响
   * 当 type 为 FORBIDDEN 或 PASS 时，使用 String[] 类型，表示待下发的数据中，如果存在这些 dp ，则不下发
...
export interface DpMapItem {
   ...
   * string 字符串
   * child 子级数据
   * complex 复合属性，使用此类型时，需要配置 childMap 或 parse 和 format。
