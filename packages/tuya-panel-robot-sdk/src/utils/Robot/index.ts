...
export function toFixed16(v: string, length = 2): string {
 ...
 */
export function decodeColor(color: string): number[] {
  let rgb;
 ...
 * @param {Number} alpha 0-1
 */
export function hex2ahex(hex: string, alpha = 1): string {
  const alpha16 = toFixed16(Math.round(alpha * 255).toString());
  const color = hex.replace(hashReg, '');
  return `#${alpha16}${color}`;
}

export function isRobotQuiet(status: string): boolean {
